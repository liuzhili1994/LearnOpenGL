...
int main() {
	...
	while (!glfwWindowShouldClose(myWindow))
	{
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			rotMat = glm::rotate(rotMat, (float)glfwGetTime() * glm::radians(50.0f), glm::vec3(0.5f, 1.0f, 0.0f));
			
			rotMat = glm::translate(rotMat, cubePositions[i]);
			float angle = 20.0f * i;
			rotMat = glm::rotate(rotMat, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
